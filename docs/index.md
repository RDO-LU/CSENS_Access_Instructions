# Instructions for researchers wanting access to COSMOS-SENS at LUNARC.
...
The GDPR is an EU directive that protects the personal information and data of individuals, therefore any identifying data from living subjects has to be kept securely under these rules. COSMOS-SENS is a isolated HPC platform housed at LUNARC where we handle and analyse data of a sensitive nature under GDPR rules. This also covers data which is sensitive for national security. ***This means COSMOS-SENS does not have a connection to the internet***, and transferring data to/from COSMOS-SENS is done via sftp (more later).

## Who is COSMOS-SENS for?

Before requesting an account, you should be comfortable using a LINUX based high-performance computing (HPC) platform.

!!! note inline end "Note"

    **COSMOS-SENS is not for general purpose file storage.** It is for data being actively worked on. for general storage please contact the RDO.

## Nodes
COSMOS-SENS has a mix of nodes available. They are:
...
### 1) Make an account at SUPR and request a COSMOS-SENS project

The PI needs to do the following.

First, make an account at [SUPR](https://supr.naiss.se/). Be sure to provide a correct mobile phone number since your phone will be needed in the future for two-factor authentication. When in SUPR, apply for a COSMOS-SENS project. [Rounds -> Special Rounds -> Rounds at LUNARC -> COSMOS-SENS]. Apply for a storage project too if you will be working on a large volume of data.

When your project is approved you will have the ability to add co-workers to your project. **Each of your co-workers will need to make an account on SUPR before they can be added**.

All are required to do the following after making SUPR accounts:
...
### 3) Request a Static IP address (**This is important, do this straight after step 2**)

As a further layer of security we employ IP address filtering to make sure only authorised users can establish a connection to COSMOS-SENS. While your LUNARC account is being created, contact LDC (servicedesk@lu.se) and request a VPN with a static IP address. When you receive your IP address, ask the project leader to raise a support ticket (support@lunarc.lu.se) with LUNARC stating what the IP address is, and who it is for. If you use a workstation connected by a cable, ask LDC to fix the IP address for your port number. **Communicate this IP address to LUNARC the same way**.


### 4) Sign the agreement

**LUNARC will send you a digital agreement that you will need sign via EduSign**. Both you and the project leader will need to do this. Someone from LUNARC will contact you with details on how you create your password.

### 5) Setup 2FA

LUNARC uses two-factor authentication using Pocket Pass on your smartphone. Follow the instructions [here](https://lunarc-documentation.readthedocs.io/en/latest/getting_started/authenticator_howto/) to set this up. 
...
!!! danger inline end "WARNING"
    **DO NOT SHARE YOUR PASSWORD! This volates the terms of the agreement and your account will be disabled.**

## Connecting to COSMOS-SENS


!!! note inline end
    **Do not forget to activate your static VPN before you try to connect**

The only way to interface with COSMOS-SENS is via a Linux desktop. Point your browser to:

[https://sensXX.lunarc.lu.se](https://sensXX.lunarc.lu.se) where XX will be the group number supplied to you by LUNARC.

Login with your **LUNARC** credentials (not Lucat) using two-factor authentication via Pocket Pass. You will be presented with the option of two GNOME desktops you can use. Pick the one you prefer. 

## Getting data to COSMOS-SENS

Getting data to COSMOS-SENS can **only** be done via sftp via the diode (**cs-diode.lunarc.lu.se**). This can made easier by mounting a COSMOS-SENS folder to folder on the local machine you are working on. Files can then be copied/moved to this mounted folder which will then become available on COSMOS-SENS.
...
**To be clear, all files in this mounted folder are located on COSMOS-SENS**. When you unmount `ToCSENS` that folder will be empty again. 

!!! warning inline end "Be smart"

    The point of COSMOS-SENS is to protect sensitive data, so be mindful about which folders you mount locally. For example, mounting `/home/<user>` would expose **everything** you have in your home folder to your local machine. If a bad actor were to try something malicious on your local computer, they would also have access to everything you have on COSMOS-SENS. Also, everything in your `/home` would also be exposed to any mistakes you make on you local machine.

    Mount specific folders rather than absolutely everything. Better safe than sorry.

## Cut & Paste

Let's get this out of the way now. Cut and paste is **not** allowed from COSMOS-SENS outwards, i.e you **cannot** `ctrl-c` in COSMOS and `ctrl-v` to a window on your local machine. You *can* paste *into* COSMOS using the clipboard which is located on the left side of the screen. Click on the arrow to pull it out.
...
There are two main folders on COSMOS-SENS:

After logging in you will land in your `home` folder (`/home/<userid>`). You will also have another folder called `/scale/grXX/shared/` where all members of your group can put files to share. Files cannot be shared between groups, i.e, members of gr01 cannot send files to people in gr02. The two front-ends cannot touch.

#### Is my data backed up?
**No**

Data on COSMOS-SENS does not have a backup. It is important you are comfortable in Linux environment, and so are individuals you add oto the project.
...
## Installing software
For software installation requests, raise a support ticket with LUNARC providing a link to the software that needs to be installed.

## Running jobs
When you login into COSMOS-SENS you will be located on the front-end (FE). **THE FE is not a place to run long computations**, only small interactive jobs. Long memory and processor intensive jobs must be sent to a compute node.
...
Your project will have a project ID associated with it (csens20XX-X-X format). You need this to run a job. 
...
Do not forget to load the CUDA modules if you need them:

```shell
module load CUDA/12.1.1
```
There are fewer GPU nodes so the queue can be long for these. 


## Run jobs using the local disk!
Most jobs create temporary files when running, so it makes sense (where possible) to use the local disk on each compute node to improve speed and reduce network traffic between the node and the storage.

SLURM is set up to give you a temporary folder on the nodes for each job you start. The name of your temporary folder is stored in the `$SNIC_TMP` variable. This variable does not exist on the frontend, and the folder on the node is private to you and will be deleted once the job is finished. So in the last step of your script you need to save your results. For example, your script could look something like:
...
cp $SNIC_TMP/out/MyAnalysis_* /home/<userid>/pathtofinaldestination/
```

So in this case, the output is directed to the **local** disk at `$SNIC_TMP/out/MyAnalysis_`, and once mapping is complete the results are copied from there to your main storage in `home`.

## Be nice to fellow co-workers

Compute resources are sometimes limited, therefore try to use only the resources as you actually need for your jobs. Make sure you read the documents of the software you are using, and whether it can actually handle multi-core processes. There is no point in asking for 10 cores if your software cannot handle it.


## Reproducible research

**Never update packages in the middle of a project!**. There is always a chance underlying methods change that may cause a change in the results. The best thing you can do is "freeze" your packages buy putting everything into a Apptainer (new name for Singularity) container/environment that you can use knowing that all package version are the same. Apptainer containers are also great for getting software onto COSMOS-SENS without needing LUNARC support to do an install. You can read more about [Apptainer images here](https://apptainer.org/docs/user/main/quick_start.html). Apptainer is installed on COSMOS-SENS and no modules are needed to load it.

Apptainer images are very useful for packaging together workflows that require several programs to run one after another. Post-publication, these images can be included in the supporting material of your paper so others in your community can download it and apply it on their own data.

## COSMOS-SENS and bioinformatics

The majority of users on COSMOS-SENS are bioinformaticians processing human sequence data. There are tools that people would like to use in a secure setting, so we will go through a few here.
...
Nextflow and the [nf-core pipelines](https://nf-co.re/) are a fantastic resource for bioinformaticians. There are a multitude of workflows for various NGS applications which go into real depth regarding QC, mapping methods, annotation, and reporting. After a pipeline is run, you also get the version numbers of the software used for easy reporting in papers, but importantly, the pipeline remains the same every time you run it ensuring results are comparable.

When an nf-core pipeline is initiated, singularity images are pulled at runtime over the internet for each component program. As COSMOS-SENS is a secure server with no internet connection, pipelines and images have to be downloaded and then transferred to COSMOS-SENS beforehand.
